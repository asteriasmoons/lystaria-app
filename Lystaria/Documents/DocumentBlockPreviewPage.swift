...
struct DocumentBlockDisplayView: View {
    ...
    private func documentTextBlock(_ block: DocumentBlock, font: UIFont) -> AnyView {
        ...
        if fullLength > 0 {
            let sortedProperties = (block.inlineProperties ?? [])
            ...
            for property in sortedProperties {
                ...
                propertyTapRanges.append(
                    InlinePropertyTapRange(
                        property: property,
                        range: NSRange(location: raw.location, length: replacement.length)
                    )
                )
            }
    ...
    private func urlPropertyButton(for property: DocumentInlineProperty, font: UIFont) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 6) {
                ...
                Text(title)
                    .font(.system(size: max(10, font.pointSize * 0.68), weight: .semibold))
                    .lineLimit(1)
            }
            .foregroundStyle(.white)
    ...
    private func attributedInlinePropertyText(for property: DocumentInlineProperty, baseFont: UIFont) -> NSAttributedString {
        ...
        if property.type == .checkbox {
            return attributedCheckboxInlinePropertyText(for: property, name: name, baseFont: baseFont)
        }
    ...
    private func attributedCheckboxInlinePropertyText(
        for property: DocumentInlineProperty,
        name: String,
        baseFont: UIFont
    ) -> NSAttributedString {
        ...
        let attributed = NSMutableAttributedString(string: fullText, attributes: [
            .font: baseFont,
            .foregroundColor: UIColor(LColors.textSecondary)
        ])
        ...
        if symbolRange.location != NSNotFound {
            attributed.addAttribute(
                .font,
                value: UIFont.systemFont(ofSize: baseFont.pointSize + 2, weight: .regular),
                range: symbolRange
            )
            attributed.addAttribute(
                .foregroundColor,
                value: property.valueStorage == "true" ? UIColor(LColors.accent) : UIColor(LColors.textSecondary),
                range: symbolRange
            )
        }
...
private struct TappableRichBlockTextView: UIViewRepresentable {
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(propertyTapRanges: propertyTapRanges, onPropertyTap: onPropertyTap)
    }
    ...
    final class Coordinator: NSObject {
        ...
        private func inlineProperty(at point: CGPoint, in textView: UITextView) -> DocumentInlineProperty? {
            guard !propertyTapRanges.isEmpty else { return nil }
            ...
            for tapRange in propertyTapRanges {
                guard tapRange.range.location >= 0,
                      tapRange.range.length > 0,
                      tapRange.range.location < textStorage.length else { continue }
...
struct DocumentTablePreviewView: View {
    ...
    private var tableData: DocumentTableData {
        DocumentTableData.from(block.text)
    }
...
// Minimal read-only UITextView wrapper for attributed strings
