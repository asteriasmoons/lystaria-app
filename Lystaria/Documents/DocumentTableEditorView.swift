...
struct DocumentTableEditorView: View {
    @Bindable var block: DocumentBlock
    var documentTextColor: UIColor = UIColor(LColors.textPrimary)

    @State private var tableData: DocumentTableData = DocumentTableData()
    ...
    var body: some View {
                    ...
                    ForEach(0..<tableData.rowCount, id: \.self) { row in
                        HStack(spacing: 0) {
                            ForEach(0..<tableData.colCount, id: \.self) { col in
                                ...
                                .overlay(
                                    // Selection overlay matching block selection style
        ...
        .onAppear {
            tableData = DocumentTableData.from(block.text)
        }
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button("Apply") {
                        ...
                        // Apply to all selected cells, or just the focused cell
                        let targets: [CellID] = isSelectingCells && !selectedCells.isEmpty
                            ? Array(selectedCells)
                            : [cell]
                        for target in targets {
    ...
    private func cellHasStyle(_ type: DocumentInlineStyleType, cell: CellID) -> Bool {
    ...
    private func toggleCellStyle(_ type: DocumentInlineStyleType, cell: CellID) {
        let range = effectiveRange
        ...
        if let idx = existing.firstIndex(where: { $0.type == type && NSEqualRanges($0.nsRange, range) }) {
            ...
            existing.append(DocumentTableCellStyle(
    ...
    private func removeCellStyle(_ type: DocumentInlineStyleType, cell: CellID) {
        let range = effectiveRange
        var existing = tableData.cellStyles(row: cell.row, col: cell.col)
    ...
    private func applyLink(cell: CellID) {
        ...
        if !trimmed.isEmpty {
            existing.append(DocumentTableCellStyle(
                typeRaw: DocumentInlineStyleType.link.rawValue,
                rangeLocation: range.location,
                rangeLength: range.length,
                urlString: trimmed
            ))
    ...
    private func applyHighlight(cell: CellID) {
        ...
        existing.append(DocumentTableCellStyle(
            typeRaw: DocumentInlineStyleType.highlight.rawValue,
            rangeLocation: range.location,
            rangeLength: range.length,
            urlString: "\(fg):\(bg)"
        ))
...
final class TableCellTextView: UITextView {
    var onPaste: ((String) -> Void)?

    override func paste(_ sender: Any?) {
        super.paste(sender)
        // super.paste() updates text synchronously before returning
...
struct RichTableCellView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> TableCellTextView {
    ...
    func updateUIView(_ uiView: TableCellTextView, context: Context) {
        ...
        if uiView.isFirstResponder {
            // While editing, only update if the plain text matches (styles changed, not text)
            // This re-applies formatting without touching the cursor
    ...
    func sizeThatFits(_ proposal: ProposedViewSize, uiView: TableCellTextView, context: Context) -> CGSize? {
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        var parent: RichTableCellView
        var isUpdating = false

        init(parent: RichTableCellView) { self.parent = parent }
        ...
        func textViewDidEndEditing(_ textView: UITextView) {
            // Re-apply full attributed text (with styles) once focus leaves
